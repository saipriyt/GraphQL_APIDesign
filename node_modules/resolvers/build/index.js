...
let create = (resFn, errFn) => {
  const Resolvers = (...args) => {
    ...
    return Promisify(resFn)(...args).catch(err => {
      ...
      return Promisify(errFn)(err, args).then(resErr => {
        throw resErr || err;
      }, resErr => {
        throw resErr || err;
      });
  ...
  Resolvers.create = (chilErr, cErrFn) => create((...args) => {
    ...
    return entry.then(r => isNotNull(r) ? r : isFun(chilErr) ? Promisify(chilErr)(...args) : Promise.resolve(null));
  }, (err, args) => {
    const entry = isFun(cErrFn) ? Promisify(cErrFn)(err) : Promise.resolve(null);
    return entry.then(r => {
      if (isNotNull(r)) throw r;
      return isFun(errFn) ? Promisify(errFn)(err).then(e => {
        throw e || err;
      }, e => {
        throw e || err;
